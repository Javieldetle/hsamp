...
<style>
    .habitacion-card { transition: transform 0.2s; cursor: pointer; border: none; }
...
document.addEventListener('DOMContentLoaded', function() {
    Swal.fire({
        ...
        text: 'La habitacion ya esta ocupada en ese rango de fechas. Seleccione otra fecha u otra habitacion.',
...
document.addEventListener('DOMContentLoaded', function() {
    Swal.fire({
        ...
        text: 'La reserva se guardo correctamente.',
...
<div class="card shadow-sm mb-4 border-0">
    <div class="card-body bg-light rounded">
        <form method="GET" id="form-filtro" class="row align-items-center g-3">
            <div class="col-md-3">
                <h2 class="fw-bold mb-0"><i class="bi bi-calendar3"></i> Ocupacion</h2>
            </div>
            <div class="col-md-6">
                <div class="input-group">
                    ...
                    <a href="habitaciones.php" class="btn btn-success fw-bold border-start">
        ...
        <div class="card habitacion-card shadow-sm h-100">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    ...
                    <div class="text-end">
                        <span class="badge bg-light text-dark border"><i class="bi bi-people"></i> <?php echo (int)$capacidad; ?></span>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-bold">Disponibilidad Habitacion <span id="num-hab-modal"></span></h5>
...
<div class="modal fade" id="modalReserva" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <form id="formReservaRecep" action="acciones_reserva.php" method="POST" class="modal-content shadow-lg border-0" autocomplete="off">
            <input type="hidden" name="accion" value="crear_reserva_completa">
            <input type="hidden" name="origen" value="habitaciones">
            <input type="hidden" name="id_habitacion" id="res_habitacion_id">
            <input type="hidden" name="total" id="res_total_val" value="0">

            <div class="modal-header bg-success text-white">
                <h5 class="modal-title">Registrar Reserva - Habitacion <span id="res_num_habitacion"></span></h5>
                            ...
                            <div class="card-body">
                                <div class="row mb-3 pb-3 border-bottom">
                                    <div class="col-12">
                                        ...
                                        <div class="input-group">
                                            <input type="text" id="buscar_cliente_recep" class="form-control" placeholder="Buscar cliente..." autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false">
                                            <button class="btn btn-outline-secondary" type="button" id="btn_limpiar_cliente_recep">Limpiar</button>
                                        </div>
                                        <div id="resultados_cliente_recep" class="list-group mt-1" style="display:none; position:absolute; z-index:1000; width:94%;"></div>
                                    </div>
                                </div>
                            ...
                            <div class="card-body">
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" name="fecha_entrada" id="res_fecha_entrada" class="form-control" min="<?php echo date('Y-m-d'); ?>" required>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label small fw-bold">Fecha Salida</label>
                                        <input type="date" name="fecha_salida" id="res_fecha_salida" class="form-control" required>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label small fw-bold">Numero de Huespedes</label>
                                        <input type="number" name="num_huespedes" id="res_huespedes" class="form-control" min="1" value="1" required>
                                        <small class="text-muted">Capacidad maxima: <span id="res_capacidad_txt">0</span></small>
                                    </div>
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-12">
                                        <div class="p-3 rounded bg-dark text-white d-flex justify-content-between align-items-center">
                                            <div>
                                                ...
                                                <h3 class="mb-0 text-success fw-bold">&euro; <span id="res_total_txt">0.00</span></h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
let calendar;
const TARIFAS_RECEP = <?php echo json_encode($tarifas_js); ?>;
...
function abrirReserva(idHabitacion, numeroHabitacion, capacidadHabitacion) {
    ...
    calcularTotalReservaRecep();
    new bootstrap.Modal(document.getElementById('modalReserva')).show();
}

function calcularTotalReservaRecep() {
    ...
    if (fEntrada && fSalida && huespedes > 0) {
        ...
        if (noches > 0) {
            let tarifa = TARIFAS_RECEP[huespedes];
            if (tarifa === undefined) {
                const keys = Object.keys(TARIFAS_RECEP).map(Number);
                const maxKey = keys.length ? Math.max(...keys) : 1;
                tarifa = TARIFAS_RECEP[maxKey] || 0;
            }
...
document.getElementById('res_fecha_entrada').addEventListener('change', calcularTotalReservaRecep);
document.getElementById('res_fecha_salida').addEventListener('change', calcularTotalReservaRecep);
document.getElementById('res_huespedes').addEventListener('input', function() {
    const v = parseInt(this.value || '0', 10);
    if (capacidadHabitacionActual > 0 && v > capacidadHabitacionActual) {
        this.value = capacidadHabitacionActual;
    }
    if (v <= 0) this.value = 1;
    calcularTotalReservaRecep();
});

document.getElementById('formReservaRecep').addEventListener('submit', function(e) {
    const fEntrada = document.getElementById('res_fecha_entrada').value;
    const fSalida = document.getElementById('res_fecha_salida').value;
    const huespedes = parseInt(document.getElementById('res_huespedes').value || '0', 10);
...
const buscarInput = document.getElementById('buscar_cliente_recep');
const listaResultados = document.getElementById('resultados_cliente_recep');
        ...
        .then(data => {
            ...
            data.forEach(p => {
                ...
                item.onclick = function(ev) {
                    ev.preventDefault();
                    document.querySelector('#formReservaRecep input[name="documento"]').value = p.documento || '';
                    document.querySelector('#formReservaRecep input[name="nombre"]').value = p.nombre || '';
                    document.querySelector('#formReservaRecep input[name="apellidos"]').value = p.apellidos || '';
                    document.querySelector('#formReservaRecep input[name="email"]').value = p.email || '';
                    document.querySelector('#formReservaRecep input[name="telefono"]').value = p.telefono || '';
...
document.getElementById('btn_limpiar_cliente_recep').addEventListener('click', function() {
    buscarInput.value = '';
    listaResultados.style.display = 'none';
});
</script>
